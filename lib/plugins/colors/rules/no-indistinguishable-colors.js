...
const messages = ruleMessages(ruleName, {
  rejected: (color1, color2) =>
    `Unexpected indistinguishable colors "${color1}" and "${color2}".`,
});
...
const COLOR_PROP_REGEX =
  /\b(.*color|.*shadow|background.*|border.*|outline.*|list-style.*|fill|stroke)\b/;
    ...
    root.walkDecls(COLOR_PROP_REGEX, (decl) => {
      ...
      for (const newColorString of newColorStrings) {
        ...
        for (const colorString of Object.keys(colors)) {
          ...
          const diffAmount = colorList[0].deltaE(newColor, "2000");

          if (diffAmount < threshold) {
            report({
              result,
              ruleName,
              message: messages.rejected(colorString, newColorString),
              node: decl,
              word: newColorString,
            });
          }
