...
const { COLOR_PROP_REGEX, COLOR_VALUE_REGEX } = require("../../utils");
...
const ruleName = "@tiwariav/stylelint-config/only-predefined-colors";

const messages = ruleMessages(ruleName, {
  rejected: (lightness) =>
    `Lightness can only take values from one of ${lightness.join(", ")}`,
});
  ...
  return (root, result) => {
    const validOptions = validateOptions(
      ...
      },
      {
        ...
        possible: {
          lightness: (x) => Array.isArray(x) && typeof x === "number",
        },
      }
    );
    const {
    ...
    root.walkDecls(COLOR_PROP_REGEX, (decl) => {
      ...
      for (const newColorString of newColorStrings) {
        ...
        if (
          ...
          newColor.lch[0] &&
          !lightness.includes(newColor.lch[0])
        ) {
          report({
            result,
            ruleName,
